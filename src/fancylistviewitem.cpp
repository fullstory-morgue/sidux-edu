...
void FancyListViewItem::paintCell( QPainter *painter, const QColorGroup &cg, int column, int width, int align)
{
	painter->save();
	...
	QCheckListItem::paintCell(painter, grp, column, width, align);
	painter->restore();
}
...
int FancyListViewItem::width(const QFontMetrics &fm, const QListView *lv, int column) const
{
	...
	if (column >= 0 && column < (int)fonts.size()) {
			...
			width = QCheckListItem::width(fm2, lv, column);
	}
	else
			width = QCheckListItem::width(fm, lv, column);
	return width;
}
